  ...
  const char** param, int num_param, const Box& box, const int number_of_atoms)
{
  ...
  //morse parameters
  float morse_dd = std::stof(param[2]); 
  float morse_a = std::stof(param[3]);
  float morse_z0 = std::stof(param[4]);
  float morse_de = std::stof(param[5]);
  float morse_half_width = std::stof(param[6]);
  float morse_half_caxis = std::stof(param[7]);
...
#endif
    if (num_gpus == 1) {
      potential.reset(new NEP3(param[1], number_of_atoms, morse_dd, morse_a, morse_z0,
      morse_de, morse_half_width, morse_half_caxis));
    } else {
      ...
      potential.reset(new NEP3_MULTIGPU(num_gpus, param[1], number_of_atoms, partition_direction, morse_dd, morse_a, morse_z0,
      morse_de, morse_half_width, morse_half_caxis));
    }
    is_nep = true;
