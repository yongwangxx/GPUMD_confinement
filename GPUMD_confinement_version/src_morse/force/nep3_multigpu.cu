...
NEP3_MULTIGPU::NEP3_MULTIGPU(
  ...
  const int num_atoms,
  const int partition_direction_input, float dd, float a,
  float z0, float de, float half_width, float half_caxis)
{
  ...
  morse_z0 = z0;
  morse_de = de;
  morse_half_width = half_width;
  morse_half_caxis = half_caxis;
  // nep3 1 C
...
static __global__ void find_force_confine(  
  ...
  float morse_z0,
  float morse_de,
  float morse_half_width,
  float morse_half_caxis
  )
  {
  ...
  GPU_Vector<double>& virial)
{
  ...
  for (int gpu = 0; gpu < paramb.num_gpus; ++gpu) {
    ...
    find_force_confine<<<
      (nep_data[gpu].N2 - nep_data[gpu].N1 - 1) / 64 + 1, 64, 0, nep_data[gpu].stream>>>(
        nep_temp_data.num_atoms_per_gpu,  nep_data[gpu].N1, nep_data[gpu].N2, 
        nep_data[gpu].position.data() + nep_temp_data.num_atoms_per_gpu * 2,
        nep_data[gpu].force.data() + nep_temp_data.num_atoms_per_gpu * 2,
        nep_data[gpu].potential.data(), morse_dd, morse_a, morse_z0, morse_de, 
        morse_half_width, morse_half_caxis);
    CUDA_CHECK_KERNEL  
